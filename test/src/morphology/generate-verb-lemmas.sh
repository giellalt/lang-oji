#!/bin/bash
# test/src/morphology/generate-verb-lemmas.sh.  Generated from generate-verb-lemmas.sh.in by configure.
...
/Users/dabowers/giella/gtcore/scripts/extract-lemmas.sh \
    $source_file > $lemmas
...
for f in $fsttype; do
    if [ $f == "xfst" ]; then
        ...
        if ! [[ $lookup_quiet == *"USAGES"* ]] ; then
            ...
            lookup_tool="/usr/local/bin/lookup -q -flags mbTT"
        fi
    elif [ $f == "hfst" ]; then
...
###### Test non-comopunds: #######
        # generate verbs in infitive, extract the resulting generated lemma,
        # store it:
